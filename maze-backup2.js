...
var render = function(){
	...
	let X = Math.round(eyeX*1000)/1000;
	let Y = Math.round(eyeY*1000)/1000;
	let diffX = FinX+.5-X;
	let diffY = FinY+.5-Y;
   	let degtheta=Math.round(theta/Math.PI*180 *10000)/10000
   	let direction = ((degtheta %360 /90)+5 )%4;		
	let dirX = (direction==1)-(direction==3);
    let dirY = (direction==2)-(direction==0);
	if (((diffY == 0 && diffX == dirX) || (diffX == 0 && diffY == dirY))
       && (maze[Y-.5][X-.5][direction] == 1 || (Math.abs(diffX) < 1 && Math.abs(diffY) < 1))) {
    ...
    for (i=0;i<polypos.length;i++){
		...
		let polyX = Math.round((polypos[i][0]) * 1000) / 1000;
		let polyY = Math.round((polypos[i][1]) * 1000) / 1000;
		let diffX = polyX + .5 - X;
		let diffY = polyY + .5 - Y;
		if (((diffY == 0 && diffX == dirX) || (diffX == 0 && diffY == dirY))
			&& (maze[Y - .5][X - .5][direction] == 1 || (Math.abs(diffX) < 1 && Math.abs(diffY) < 1))) {
...
function nextMove(theta,X,Y,dtheta,dX,dY,isRat=false)
{
	...
	if (degtheta %90) {
		...
		//
		//strategy: 
		//rat just-turned and front=open - foward
		//right=open - rotate right
		//front=open - foward
		//left=open - rotate left
		//else i.e. dead end - rotate right
		if (dtheta && !dY && !dX && walls[direction]==1) {
			dX = ((direction == 1) - (direction == 3)) / (isRat ? 50 : 100);
			dY = ((direction == 2) - (direction == 0)) / (isRat ? 50 : 100);
			return [theta,X+dX,Y+dY,0,dX,dY];
		} else if (walls[(direction+3)%4]==1 && moveNumber > 1) {
			dtheta = -1 * Math.PI / (isRat ? 20.0 : 200.0);
        	let dirX = (direction==2)-(direction==0);
        	let dirY = (direction==3)-(direction==1);
			let turnInPlace = isRat || (polypos.find((el) => el[0] == X + dirX - .5 && el[1] == Y + dirY - .5)) || (X+dirX == FinX+.5 && Y+dirY == FinY+.5);
			dX = turnInPlace ? 0 : dirX/100;
			dY = turnInPlace ? 0 : dirY/100;
			return [theta+dtheta,X+dX,Y+dY,dtheta,dX,dY];
			...
			dtheta = 1 * Math.PI / (isRat ? 20.0 : 200.0);
			let dirX = (direction == 0) - (direction == 2);
			let dirY = (direction == 1) - (direction == 3);
			let turnInPlace = isRat || (polypos.find((el) => el[0] == X + dirX - .5 && el[1] == Y + dirY - .5)) || (X+dirX == FinX+.5 && Y+dirY == FinY+.5);
			dX = turnInPlace ? 0 : dirX/100;
			dY = turnInPlace ? 0 : dirY/100;
			return [theta+dtheta,X+dX,Y+dY,dtheta,dX,dY];
